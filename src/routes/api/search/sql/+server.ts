...
export const POST: RequestHandler = async ({ request, locals }) => {
	try {
		...
		// Genre/subgenre names live in Supabase; resolve filter values against them
		const taxonomy = await getTaxonomy(supabase);

		// Apply filters based on what's present

		// 1. Period filter - works.period column. Normalize each value to a
		// canonical period (handles "Romantic Period", "modern", etc.), then
		// match against the variant spellings stored in works.period.
		if (filters.period) {
			const requested = Array.isArray(filters.period) ? filters.period : [filters.period];
			const periods = [...new Set(requested.map((p) => normalizePeriod(p) ?? p))];

			const orConditions = periods.flatMap((p) => {
				const variants = PERIOD_VARIANTS[p as keyof typeof PERIOD_VARIANTS] ?? [p];
				return variants.map((v) => `period.ilike.%${v}%`);
			});

			if (orConditions.length === 1) {
				query = query.ilike('period', orConditions[0].replace('period.ilike.', ''));
		...
		// 3. Genre filter - resolve names against the genres table (case- and
		// whitespace-insensitive), filter by genre_id, and also check the
		// legacy 'genre' text column for backwards compatibility
		if (filters.genre) {
			const genres = Array.isArray(filters.genre) ? filters.genre : [filters.genre];
			const orConditions: string[] = [];

			for (const genreName of genres) {
				const genreRow = matchTaxonomyName(taxonomy.genres, genreName);
				if (genreRow) {
					// Search both genre_id (foreign key) and genre (text) columns
					orConditions.push(`genre_id.eq.${genreRow.id}`);
				}
		...
		// 4. Subgenre filter - resolve names against the subgenres table and
		// filter by subgenre_id
		if (filters.subgenre) {
			const subgenres = Array.isArray(filters.subgenre) ? filters.subgenre : [filters.subgenre];
			const subgenreIds = subgenres
				.map((name) => matchTaxonomyName(taxonomy.subgenres, name)?.id)
